...
from app.schemas import Token, RefreshTokenRequest, LoginRequest
...
async def login(credentials: LoginRequest, db: AsyncSession = Depends(get_db)) -> Token:
    return await auth_service.authenticate_user(db=db, email=credentials.email, password=credentials.password)
